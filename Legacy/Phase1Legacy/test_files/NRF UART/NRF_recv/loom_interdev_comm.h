...
// sprintf(addressString, "%s%s", packet_header_string, "/DeviceChannelPoll");
	sprintf(addressString, "/%s%s", LOOM_FAMILY, "/DeviceChannelPoll");
...
void respond_to_device_poll(OSCMessage &msg) 
{
	...
	sprintf(addressString, "/%s%s", LOOM_FAMILY, "/DevicePollResponse");

	char buf[20];
	sprintf(buf, "%s%d", DEVICE, configuration.instance_number);
	OSCMessage tmpMsg = new OSCMessage(addressString);
	tmpMsg.add(buf);
// Should the bundle be addressed to just LOOM_FAMILY, or the device that send the request?
