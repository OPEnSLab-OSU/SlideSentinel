...
char RTKString[NRF_MESSAGE_SIZE*15];
int len;
...
void recvNRF(char * string_to_fill, int *len){
  ...
  *len = 0;
  ...
  if(network.available()){
    ...
    RF24NetworkHeader header(NRF_NODE_ADDRESS, 'A');          // If so, grab it and print it out
...
void sendNRF(char * string_to_send, int send_len){
  ...
  char message[NRF_MESSAGE_SIZE];
  RF24NetworkHeader header(NRF_NODE_ADDRESS);              // This should be better generalized, as to be able to send to nodes
