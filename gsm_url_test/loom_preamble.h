...
#define PacketHeaderString STR(/) LOOM_FAMILY STR(LOOM_FAMILY_NUM) STR(/) DEVICE // Results in a single string, i.e. /LOOM/Device. the full prefix sent to this device should be /LOOM/Device#, but the number is parsed in the OSC bundle routing function
...
// --- WiFi UDP Ports ---
#if is_wifi == 1
	#define SUBNET_PORT     GLOBAL_PORT+(10*LOOM_FAMILY_NUM)	
#endif

// If using channels
#if LOOM_CHANNEL >= 1 && LOOM_CHANNEL <= 8
	#define INIT_INST LOOM_CHANNEL
	#define INIT_PORT SUBNET_PORT+LOOM_CHANNEL
#endif
