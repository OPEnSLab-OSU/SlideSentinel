 ...
 */
void GNSSController::sbp_setup(void) {
...
uint8_t GNSSController::getFixMode(msg_pos_llh_t pos_llh, char rj[]) {
...
u8 GNSSController::fifo_empty(void) {
  if (head == tail)
    return 1;
  return 0;
}
 ...
 */
u8 GNSSController::fifo_write(char c) {
  if (fifo_full())
    return 0;
 ...
 */
u8 GNSSController::fifo_read_char(char *c) {
  if (fifo_empty())
    return 0;
 ...
 */
u32 GNSSController::fifo_read(u8 *buff, u32 n, void *context) {
  int i;
  for (i = 0; i < n; i++)
...
/* Return 1 if true, 0 otherwise. */
u8 GNSSController::fifo_full(void) {
...
void GNSSController::poll(Serial_ &terminal) {
  GNSSread();
