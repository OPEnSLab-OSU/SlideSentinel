...
void TestInsertion() {
    int dcount = 0;
    {
        ...
        TEST_ASSERT_EQUAL_MESSAGE(8, buf.size(), "miscalculated size!");
    ...
    TEST_ASSERT_EQUAL_MESSAGE(0, dcount, "improper cleanup");
}
...
void TestRemove() {
	...
	++iter; ++iter; ++iter; ++iter;
	m_buf.remove(iter);
	TEST_ASSERT_EQUAL(7, m_buf.size());
	...
	++citer; ++citer; ++citer;
	m_buf.remove(citer);
	TEST_ASSERT_EQUAL(6, m_buf.size());
...
void TestFront() {
    ...
    TEST_ASSERT_EQUAL(m_answer[0], m_buf.front().m_i);
	TEST_ASSERT_EQUAL(m_answer[7], m_buf.back().m_i);
}
